...
### 10. Вход не проходит: `net.openid.appauth.AuthorizationException: Invalid ID Token`

**Симптом:** форма логина Keycloak принимает логин/пароль без ошибок
(в отличие от проблемы 9b — до формы и обратно в приложение всё
доходит), но вместо экрана профиля студента — красный тост/лог с
`AuthorizationException: Invalid ID Token`, и `ReactNativeJS` в logcat
показывает `Uncaught (in promise, id: 0): "net.openid.appauth.AuthorizationException: Invalid ID Token"`.
Реальный логин/пароль тестового студента при этом верны — смена
пароля или пересоздание пользователя в Keycloak не помогает.

**Причина:** у эмулятора рассинхронизировались системные часы —
обычно после того, как хост уходил в сон/гибернацию, пока эмулятор
был запущен, или эмулятор стартовал из старого сохранённого снапшота
(`quick-boot`), «заморозившего» гостевые часы на момент снапшота.
`react-native-app-auth` (через AppAuth-Android) валидирует claim `iat`
(issued-at) в ID-токене, который Keycloak подписывает по *своему*
(правильному) времени; если часы эмулятора отстают от реального
времени больше чем на несколько минут, токен выглядит «выпущенным в
будущем» и AppAuth отбрасывает его с этой ошибкой — сам обмен
authorization code на токен (в отличие от 9b) при этом проходит
успешно, чем и объясняется несовпадение с проблемой 9b. Проверить:
```sh
adb shell date
date -u
```
Если разница больше пары минут — это оно.

**Решение:** холодный перезапуск эмулятора, чтобы гостевые часы
переинициализировались от хоста, а не восстановились из снапшота:
```powershell
adb -s emulator-5554 emu kill
# дождаться, пока qemu-system-x86_64.exe исчезнет из списка процессов
emulator -avd Pixel_10 -no-snapshot-load
```
`adb shell date -s ...` эту проблему не чинит — `Pixel_10` (как и
большинство образов с Play Store) собран как production-билд, где
`adbd` не может выполниться от root («adbd cannot run as production
builds»), а без root установка системного времени запрещена.
После холодного старта не забыть заново поднять `adb reverse` для
8081/8443 (см. проблему 9a и общий рецепт ниже) — они слетают при
каждом перезапуске эмулятора.
